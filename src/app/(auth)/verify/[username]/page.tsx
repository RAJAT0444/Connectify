...
// ✅ Define a message-only response type
...
export default function VerifyAccount() {
  ...
  const onSubmit = async (data: z.infer<typeof verifySchema>) => {
    setLoading(true);
    try {
      const response = await axios.post<ApiResponse<MessageResponse>>(`/api/verify-code`, {
        username: params.username,
        code: data.code,
      });
      ...
      const axiosError = error as AxiosError<ApiResponse<MessageResponse>>;
      toast.error(
        axiosError.response?.data.message ?? 'An error occurred. Please try again.',
        { icon: '❌' }
      );
  ...
  const handleResendCode = async () => {
    setResendDisabled(true);
    try {
      await axios.post<ApiResponse<MessageResponse>>(`/api/resend-verification-code`, {
        username: params.username,
      });
      toast.success('Verification code resent!');
    } catch (error) {
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="flex justify-between items-center mt-8">
              ...
              >
                <RotateCw
                  className={`mr-2 h-4 w-4 ${resendDisabled ? 'animate-spin' : ''}`}
                />
                {resendDisabled ? `Resend in ${countdown}s` : 'Resend Code'}
              </Button>
